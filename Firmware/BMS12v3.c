// BMS12v3: 12-cell Lithium Battery Management Module V3
...
#define USE_29BIT_IDS	1   // or legacy 11-bit IDs if set to zero

// Millivolts added to sampled voltage, to allow for voltage drop through buffer resistors
// and calibrating for manufacturing tolerances between LTC6802 chips. 6 is about average.
#define CORRECTION		6
...
#define BASE_ID	300 // Starting CAN bus message ID.
...
// SPI interface pins
#define	CSBI		(1<<PB3)
#define CSBI_PORT	PORTB
#define	SDO			(1<<PC6)
#define SDO_PORT	PINC
#define SDI			(1<<PB4)
#define SDI_PORT	PORTB
#define SCKI		(1<<PC7)
#define SCKI_PORT	PORTC
...
#define	MOD_ID_NUM8		(PIND & (1<<PD7))
#define MOD_ID_NUM4		(PINC & (1<<PC4))
#define MOD_ID_NUM2		(PINC & (1<<PC5))
#define MOD_ID_NUM1		(PINB & (1<<PB2))
...
// Global variables
uint8_t txData[8];
volatile uint8_t dataRequested = false;
uint16_t moduleID = 0;
short commsTimer = 0;
volatile int temp[2]; // In deg C
volatile int voltage[12]; // In millivolts
volatile uint16_t shuntVoltage; // In millivolts
uint16_t shuntBits;

// Interrupt function called when a new CAN message is received
SIGNAL(CAN_INT_vect)
{
	uint8_t rxData[8];
	...
	if (CANSTMOB & (1<<RXOK))
	{
		...
		if (rxPacketID == moduleID + BMS12_REQUEST_DATA) // This enum is actually zero but here for clarity
		{
...
int main()
{
	...
	for (int n=0; n<12; n++) voltage[n] = 0;
	for (int n=0; n<2; n++) temp[n] = 0;
	...
	while (1)
	{
		...
		WriteSPIByte(((unsigned short)(shuntBits & 0x00FF))); // Bottom byte of shunt bits
		...
		WriteSPIByte(RDCV);
		SDI_PORT |= SDI;
		for (int n=0; n<25; n++) cellBytes[n] = ReadSPIByte();
		CSBI_PORT |= CSBI;
		_delay_us(100);
		...
		WriteSPIByte(RDTMP);
		SDI_PORT |= SDI;
		for (int n=0; n<5; n++) tempBytes[n] = ReadSPIByte();
		CSBI_PORT |= CSBI;
		_delay_us(100);
		...
		{
			voltages[n][counter] = (cellBytes[n*3/2] + 256 * (cellBytes[n*3/2+1] & 0x0F))*3/2;
			voltages[n+1][counter] = (((cellBytes[n*3/2+1] & 0xF0)>>4) + cellBytes[n*3/2+2]*16)*3/2;
		}

		tempBuffer[0][counter] = tempBytes[0] + 256 * (tempBytes[1] & 0x0F); // gives mV, convert to degrees later
		tempBuffer[1][counter] = ((tempBytes[1] & 0xF0)>>4) + tempBytes[2]*16;
		...
		if (counter >= 8) // Slow loop, about 4Hz
		{
			...
			for (int n=0; n<12; n++) // Calculate average voltage over last 8 samples, and update shunts if required
			{
				int average = 0;
				...
				if (voltage[n] > 0)
				{
					...
					if (n==0) voltage[n] -= CORRECTION/3; // First cell has less drop due to single 3.3Kohm resistor in play
				}
			...
			// Calculate temperature averages over last 8 samples
			for (int n=0; n<2; n++)
			{
				int average = 0;
		...
		if (commsTimer < COMMS_TIMEOUT)
			...
			shuntVoltage = 0; // If comms times out, so we don't keep balancing with an outdated threshold

		if (dataRequested)
		{
			dataRequested = false;
			commsTimer = 0; // Reset comms timeout
			...
			// Temperature packet
			txData[0] = LineariseTemp(temp[0]); // Converts to degrees (with +40 offset)
...
// From datasheet for Epcos 100Kohm NTC B25/100 of 4540 K
...
void SetupPorts()
{
	DDRB = 0b00011001; // PB0 = CAN Enable, PB3 = CSBI, PB4 = SDI
	DDRC = 0b10000010; // PC7 = SCKI, PC1 = GREEN LED
	DDRD = 0b00001000; // PD3 = RED LED
